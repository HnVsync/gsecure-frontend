...
function SavedPassword() {
    ...
    const handleCopyPassword = async (encryptedPassword) => {
        ...
        try {
            ...
            alert("Failed to decrypt password. Please check your keyword.");
        }
    };
            ...
            <div className="w-full">
                <div className="max-w-5xl mx-auto p-4 sm:p-6">
                    ...
                    {passwords.length > 0 ? (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            {passwords.map((password) => (
                                <div
                                    key={password.id}
                                    className="rounded-lg shadow-md p-6 border border-gray-200 hover:shadow-lg transition"
                                >
                                    <div className="mb-4">
                                        ...
                                        <p className="text-gray-500 mt-1">Username: {password.username}</p>
                                    </div>
                                    ...
                                    <div className="mb-4 relative">
                                        ...
                                        <button
                                            ...
                                            disabled={!password}
                                            className="absolute inset-y-0 right-0 pr-3 cursor-pointer inline-flex items-center px-4 py-2 text-gray-700 disabled:opacity-50 disabled:cursor-not-allowed rounded-r-lg"
                                        >
                                    ...
                                    {password.notes && (
                                        <div className="mb-4 p-3 bg-yellow-950 rounded-lg border border-yellow-200">
                                            <p className="text-sm text-yellow-100"><strong>Note :</strong> {password.notes}</p>
                                        </div>
                                    )}
                                </div>
                            ))}
                        ...
                        <div className="rounded-lg shadow-md p-8 text-center">
                            <h3 className="text-lg font-medium text-gray-400 mb-2">No passwords saved yet</h3>
                    ...
                    <div className="flex gap-1 text-center">
                        <button
                            className="px-6 py-2 bg-yellow-950 hover:bg-yellow-900 text-white rounded-lg transition"
                            disabled={isLoading}
                            onClick={getSavedPassword}
                        >
                            {isLoading ? "Loading..." : "Fetch Saved Passwords"}
                        </button >
                        <button className='px-6 py-2 bg-yellow-950 hover:bg-yellow-900 text-white rounded-lg transition' onClick={handleAdd}>Add Password</button>
                    </div>
                </div>
            </div>
